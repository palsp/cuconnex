...
export async function compile(cc: CompilerContext, preset: Preset) {
  ...
  const compiledFileTypes = preset.getDynamicConfig
    ...
    : [];
  if (
    getNonCompiledFiles(cc.files, compiledFileTypes).length === cc.files.length
  ) {
...
async function _compile(
  ...
  await runCompiler(
    () =>
      runNodeScriptInDir(context.directory, pathToCompiler, compilerArguments),
    preset
  );
...
export function getNonCompiledFiles(
  files: Vinyl[],
...
export function findMainFile(context: CompilationContext, dists: Vinyl[]) {
  ...
  const pathPrefix = `${compDistRoot}${
    compDistRoot.endsWith(path.sep) ? "" : path.sep
  }`;
...
async function runNodeScriptInDir(
  directory: string,
  scriptFile: string,
  args: string[]
) {
...
async function createConfigFile(context: CompilationContext): Promise<any> {
  ...
  const content: GenericObject = context.cc.dynamicConfig![
    getNameOfFile(configFileName, ".")
  ]; //the key of the config file is defined by configFileName and can be dynamic.
...
export async function collectDistFiles(
  ...
  return files.map((file, index) => {
    const relativePath = path.relative(
      path.join(capsuleDir, FIXED_OUT_DIR),
      file
    );
...
async function collectNonDistFiles(
  context: CompilationContext
): Promise<Vinyl[]> {
...
function isTestFile(
  srcTestFiles: Vinyl[],
  fileToCheck: string,
  compareWithExtension: boolean = true
) {
...
export function getCapsuleName(ctx: CompilerContext) {
  ...
  const componentInProjectId = md5(
